...
import { handlePlay, cleanEpisodeTitle } from "../../lib";

const screenName: ScreenName = "Seasons"
type Props = ScreenProps[typeof screenName];

function Seasons({ mediaCard, onBack, onProfileClick}: Props) {
  const [seasons, setSeasons] = useState<Array<[number, [MediaCard, MediaCard[]]]> | null>(null);  //[seasonNum, [seasonCard, [episodeCards..]]]
          ...
          <div className="seasons__right">
            ...
            <div className="seasons__episodes-list--wrap">
              ...
              <div className="seasons__episode-row" key={epCard.episodeOverallNumber}>
                ...
                <button className="btn btn--sm btn--circle btn--play-episode" title="Play" onClick={() => handlePlay(epCard)}>▶</button>
                <span className="seasons__episode-title">{cleanEpisodeTitle(epCard.title)}</span>
